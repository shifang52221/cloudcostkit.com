...
test("tertiary calculator and hub batch keeps richer and distinct meta descriptions", () => {
  const cases = [
    ...
    "src/pages/calculators/finops.astro",
    "src/pages/calculators/kubernetes.astro",
  ...
  const descriptions = cases.map((file) => {
    ...
    assert.ok(description.length >= 145, `${file} description should be at least 145 chars, got ${description.length}`);
  ...
  assert.equal(new Set(descriptions).size, descriptions.length, "tertiary calculator descriptions should remain distinct");
});
