...
- No fluff: do not pad word count.
- Unique information points: each indexable page must include **8+ distinct, page-specific** information points (facts, steps, decisions, pitfalls, validation checks, or worked examples).
- Intent separation: similar slugs must have clearly different intent (workflow vs checklist vs mistakes vs validation), not the same outline with synonyms.

## Minimum standard for new guides
- Clear intent: answer one specific query (pricing / estimate / optimize / compare).
- Structure: at least 5+ meaningful sections (not filler).
...
- FAQ: 3+ real questions that cover long-tail variants.
- Internal links: link to 3+ relevant calculators/guides and be linked back from the cluster hub.
...
- “Next steps” (links to a hub + 2+ deep guides)
- Sources: link to official pricing/docs and state assumptions (region, units, tiers).

## Release workflow (per batch)
- Add a hub page that links the cluster (pricing / estimate / optimize / compare).
- Add 2+ deep pages with unique workflows or checklists.
